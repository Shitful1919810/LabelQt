...
- `ProofreadReportService`：生成校对 HTML 报告。它只负责文件内容，不弹文件选择窗口，也不读取 UI 状态。报告按页面分组，使用类似校对预览界面的“修改前 / 修改后”双图展示 marker，并始终把压缩后的页面图像以内嵌 base64 的方式写入单个 HTML 文件。同一页两侧图片路径相同时，内嵌图片只写入一次并由两个预览复用。
...
- `TextDiffService` 直接使用 `diff-match-patch` 的 `diff_main()`，随后调用 `diff_cleanupSemantic()` 和
  `diff_cleanupSemanticLossless()` 做语义清理，不再额外加入“整句删除 + 整句新增”之类的展示兜底。这样可以让校对窗口和 HTML 报告尽量忠实呈现上游 diff 算法的切分结果，后续若要调整文本 diff 粒度，应优先从 `TextDiffService` 的 cleanup 策略入手。
