...
- 当字符级 diff 因语序重排而变得碎片化，并且变动文本相对保留文本占比足够高时，`TextDiffHtmlRenderer` 会退化为“整句删除 + 整句新增”的两行展示。长段落里的局部增删、少量换行调整仍保留局部高亮，避免把小改动显示成整段重写。这个判断只影响可读性展示，不改变 `TextDiffService` 的结构化 diff 结果。
