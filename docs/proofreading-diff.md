...
- 不超过 3 个字符的短文本如果发生变化且跨序号匹配，需要达到更高配对门槛，避免“啊”“嗯”“！”这类短标签在移动、增删时被误配。
- 同序号短文本不使用额外门槛，仍可正常识别为文本修改。
- 类别相同：少量加分。
- marker 坐标接近：使用快速衰减的平滑函数加分。坐标完全重合或只有小幅移动时会给较高分，用于识别“校对时主要改文本、marker 没动或只轻微调整”的常见情况；距离继续拉开后分数会快速下降，避免把只是大致在同一区域的不同 label 误配。
- 原序号相同：少量加分。
- 原序号距离较远：少量扣分。
