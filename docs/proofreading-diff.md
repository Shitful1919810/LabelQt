...
- `ProofreadReportService`：生成校对 HTML 报告。它只负责文件内容，不弹文件选择窗口，也不读取 UI 状态。
...
- 文本完全相同：高分。
- 文本相似：按字符 LCS 相似度给分。
- 不超过 3 个字符的短文本相似但不相同：强制降低分数，避免“啊”“嗯”“！”这类短标签被误配。较长文本仍允许通过相似度被识别为修改。
- 类别相同：少量加分。
...
- 校对报告导出逻辑应继续放在 `ProofreadReportService`，对话框只负责收集保存路径和本地化文案。
